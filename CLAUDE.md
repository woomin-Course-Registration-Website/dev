...
- **프론트엔드 CD**: **Amplify**가 `main` 브랜치를 watch하여 GitHub webhook으로 자동 빌드/배포(`amplify.yml`, appRoot `frontend`). **Amplify는 콘솔에서 GitHub App으로 연결**(repo가 속한 org가 deploy key를 막아 Terraform 토큰 방식 불가) → `infra/terraform/amplify.tf`는 비활성(`amplify.tf.console-managed` 참고 보존), 브랜치/도메인은 콘솔·CLI로 관리. 커스텀 도메인 `www.kodexopmp-inu.com` 적용(DNS는 별도 AWS 계정 Route53). 기본 도메인 `https://main.<app-id>.amplifyapp.com`도 유효. GitHub Actions 워크플로우 불필요.
...
- **진입 토폴로지**: 브라우저 → Amplify(SPA, public, `www.kodexopmp-inu.com` + `*.amplifyapp.com`) / 브라우저 → API Gateway HTTP API(public, `*.execute-api.amazonaws.com`) → VPC Link → private ALB → EKS backend. **현재 dev만 라이브**(API Gateway는 `var.live_envs`로 dev만 생성; staging/prod는 ALB가 뜬 뒤 승격 시 추가). API Gateway CORS는 `var.frontend_origins`로 Amplify origin에 한정.
- **인프라**: **Terraform**(`infra/terraform/`)으로 VPC·EKS·RDS·ECR·API Gateway·Sealed Secrets controller·기본 StorageClass(gp3)를 관리(Amplify는 위 사유로 콘솔 관리). **Kustomize**(`k8s/base` + `k8s/overlays/{dev,staging,prod}`)로 Deployment·Service·HPA·PDB·Ingress·NetworkPolicy·SealedSecret·ServiceMonitor 구성.
